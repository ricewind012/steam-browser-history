...
(async () => {
    ...
    const classes = (await import("../classes.js")).default;
    const mainWindow = SteamUIStore.WindowStore.SteamUIWindows[0].m_BrowserWindow;
    const urlBar = await ElementUtils.wait(`.${classes.steamdesktop.URLBarText}`, mainWindow.document);
    urlBar.addEventListener("click", async () => {
        ...
        const wnd = await CreateWindow({
            browserType: 4 /* EBrowserType.DirectHWND_Borderless */,
            createflags: 8 /* EPopupCreationFlags.NoTaskbarIcon */ |
                262144 /* EPopupCreationFlags.NoRoundedCorners */ |
                16384 /* EPopupCreationFlags.NoWindowShadow */,
        }, {
